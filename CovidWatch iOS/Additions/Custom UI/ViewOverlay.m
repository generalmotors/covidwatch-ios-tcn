...
- (void)setup
{
	...
	if (self.parentController)
		...
		switch (self.position)
			{
			case ViewOverlayTabPositionTopEdgeLeft:
			case ViewOverlayTabPositionTopEdgeCenter:
			case ViewOverlayTabPositionTopEdgeRight:
				if (self.position == ViewOverlayTabPositionTopEdgeLeft)
					{
					xpos = 0;
					}
				else if (self.position == ViewOverlayTabPositionTopEdgeCenter)
					{
					xpos = ((frame.size.width - (IoGViewOverlaySpaceBetweenHorizontalTabs * 2)) / 3) + IoGViewOverlaySpaceBetweenHorizontalTabs;
					}
				else if (self.position == ViewOverlayTabPositionTopEdgeRight)
					{
				...
				break;
			case ViewOverlayTabPositionLeftEdgeTop:
			case ViewOverlayTabPositionLeftEdgeCenter:
			case ViewOverlayTabPositionLeftEdgeBottom:
				if (self.position == ViewOverlayTabPositionLeftEdgeTop)
					{
					ypos = 0;
					}
				else if (self.position == ViewOverlayTabPositionLeftEdgeCenter)
					{
					ypos = ((frame.size.height - (IoGViewOverlaySpaceBetweenVerticalTabs * 2)) / 3) + IoGViewOverlaySpaceBetweenVerticalTabs;
					}
				else if (self.position == ViewOverlayTabPositionLeftEdgeBottom)
					{
				...
				break;
			case ViewOverlayTabPositionBottomEdgeLeft:
			case ViewOverlayTabPositionBottomEdgeCenter:
			case ViewOverlayTabPositionBottomEdgeRight:
				if (self.position == ViewOverlayTabPositionBottomEdgeLeft)
					{
					xpos = 0;
					}
				else if (self.position == ViewOverlayTabPositionBottomEdgeCenter)
					{
					xpos = ((frame.size.width - (IoGViewOverlaySpaceBetweenHorizontalTabs * 2)) / 3) + IoGViewOverlaySpaceBetweenHorizontalTabs;
					}
				else if (self.position == ViewOverlayTabPositionBottomEdgeRight)
					{
					xpos = (((frame.size.width - (IoGViewOverlaySpaceBetweenHorizontalTabs * 2)) / 3) * 2) + (IoGViewOverlaySpaceBetweenHorizontalTabs * 2);
					}
				...
				buttonOrientationModifier = CGAffineTransformMakeRotation(0);
				break;
			case ViewOverlayTabPositionRightEdgeTop:
			case ViewOverlayTabPositionRightEdgeCenter:
			case ViewOverlayTabPositionRightEdgeBottom:
				if (self.position == ViewOverlayTabPositionRightEdgeTop)
					{
					ypos = 0;
					}
				else if (self.position == ViewOverlayTabPositionRightEdgeCenter)
					{
					ypos = ((frame.size.height - (IoGViewOverlaySpaceBetweenVerticalTabs * 2)) / 3) + IoGViewOverlaySpaceBetweenVerticalTabs;
					}
				else if (self.position == ViewOverlayTabPositionRightEdgeBottom)
					{
					ypos = (((frame.size.height - (IoGViewOverlaySpaceBetweenVerticalTabs * 2)) / 3) * 2) + (IoGViewOverlaySpaceBetweenVerticalTabs * 2);
					}
		...
		if (self.position == ViewOverlayTabPositionTopEdgeLeft || self.position == ViewOverlayTabPositionTopEdgeCenter || self.position == ViewOverlayTabPositionTopEdgeRight || self.position == ViewOverlayTabPositionBottomEdgeLeft || self.position == ViewOverlayTabPositionBottomEdgeCenter || self.position == ViewOverlayTabPositionBottomEdgeRight)
			{
...
- (void)layoutContent
{
	...
	if (self.contentType == ViewOverlayTypeList)
		{
	...
	else if (self.contentType == ViewOverlayTypeGrid)
		{
...
- (void)tabClicked
{
	...
	if (!self.contentRevealed)
		...
		if (self.delegate != nil)
			{
			[self.delegate overlayViewDisplayed:self.tabTitle];
			}
		}
	else
		{
		[UIView animateWithDuration:IoGViewOverlayDrawerSlideAnimationDuration animations:^{self.frame = self.overlayConcealedFrame;} completion:^(BOOL finished){if (self.position == ViewOverlayTabPositionTopEdgeLeft || self.position == ViewOverlayTabPositionTopEdgeCenter || self.position == ViewOverlayTabPositionTopEdgeRight || self.position == ViewOverlayTabPositionBottomEdgeLeft || self.position == ViewOverlayTabPositionBottomEdgeCenter || self.position == ViewOverlayTabPositionBottomEdgeRight){self.contentView.hidden = YES;}}];
		if (self.delegate != nil)
			{
			[self.delegate overlayViewContracted:self.tabTitle];
			}
		}
...
- (CGRect)calculateVisibleFrame
{
	...
	CGFloat         statusBarHeight = windowScene.statusBarManager.statusBarHidden ? 0 : windowScene.statusBarManager.statusBarFrame.size.height;
